...
class PantallaJuego(pilasengine.escenas.Escena):
	...
	class Asteroide(pilasengine.actores.Actor):
		...
		def iniciar(self):
			...
			self.escala = 0.18 * self.pilas.azar(1, 3)
			self.x = -500
			self.y = self.pilas.azar(-300, 300)
			self.giro = 0.5 * self.pilas.azar(1, 4)
			self.z = self.y
	...
	def cambiar_posicion_camara(self):
	    self.pilas.camara.x += self.pilas.camara.x / 20.0
	    print "CAMARA"
	    return True
	...
	def crear_asteroide(self):
		...
		asteroide = self.Asteroide(self.pilas)
	...
	def iniciar(self):
		...
		self.pilas.tareas.agregar(1/40.0, self.cambiar_posicion_camara)
		contador_texto = 0
		contador_choques = 0

		fondo = self.pilas.fondos.Galaxia(dx=-2, dy=0)
		fondo = self.pilas.fondos.DesplazamientoHorizontal()
		# MUSICA
		...
		print "POR ACA EMISOR.CONSTANTE = ", emisor.constante
	...
	def actualizar(self):
		...
		if (flag[0]) == False:
				print "NIVEL 1"
				PantallaJuego.tareaAsteroides = self.pilas.tareas.siempre(2, self.crear_asteroide)
			
				flag[0] = True
				print "ACA NO HUBO PROBLEMA"	
				cambio_nivel(1, "Leucocitos")
		...
		if contador_texto == 34:
			...
			if (flag[1]) == False:
				...
				PantallaJuego.tareaAsteroides = self.pilas.tareas.siempre(2.5, self.crear_asteroide)
				PantallaJuego.velocidad_asteroides = 6
				fondo = self.pilas.fondos.Galaxia(dx=-3, dy=0)
				fondo.imagen = ruta + '/imagenes/galaxia_01.png'
				cambio_nivel(2, "Virus")
				flag[1] = True
